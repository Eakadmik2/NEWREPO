    ...
    try:
        ...
        for stud in json_sstudent:
            ...
            attendace_report = AttendanceReport(
                student_id=student, attendace_id=attendace, status=stud['status'])
...
def save_updateattendance_data(request):
    ...
    try:
        for stud in json_sstudent:
            student = Students.object.get(admin=stud['id'])
            attendance_report = AttendanceReport.objects.get(
                student_id=student, attendace_id=attendance)
            attendance.report.status = stud['status']
            attendance.report.save()
        return HttpResponse("OK")
    except:
        return HttpResponse("Error")
...
def save_student_result(request):
    ...
    try:
        ...
        if check_exist:
            ...
            result = StudentResult(subject_id=subject_obj, student_id=student_obj,subject_exam_marks=exam_marks,subject_assignment_marks=assignment_marks)
            result.save()
            messages.success(request, "Successfully Added Result")
            return HttpResponseRedirect(reverse("staff_add_result"))
    except:
        messages.success(request, "Failed to Add Result")
        return HttpResponseRedirect(reverse("staff_add_result"))
